...
class Tree{
     ...
     * @return string
     */
    public function getTreeArray($myid, $itemprefix = '',$level = 1){
        $childs = $this->getChild($myid);
        $n = 0;
        $data = [];
        $number = 1;
        if ($childs)
        {
            ...
            foreach ($childs as $id => $value){
               ...
               if ($number == $total){
                ...
                $spacer = $itemprefix ? $this->nbsp.$this->nbsp.$itemprefix . $j.$this->nbsp : '';
                $value['spacer'] = $spacer;
				$value['level'] = $level;
                $data[$n] = $value;
                $data[$n]['childlist'] = $this->getTreeArray($id, $itemprefix . $k .$this->nbsp.$this->nbsp, ($level + 1));
                $n++;
                $number++;
            }
        }
    ...
    public function getTreeList($data=[],$field = 'name'){
        ...
        foreach ($data as $k => $v){
            ...
            unset($v['childlist']);
			$v['level_show'] = $v['spacer'];
			$v['title_show'] = $v['level_show'].$v[$field];
