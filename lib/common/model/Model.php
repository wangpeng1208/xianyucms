...
class Model extends \think\Model{
			...
			if($data){
				if (empty($data['id'])) {
					...
					if ($data['extend'] == 1) {
						...
						$dataid=$data['name']."_id";
						$sql = $db->start_table($data['name'])->create_id($dataid, 11 , '主键' , true)->create_uid()->create_key($dataid);
					}
					//执行操作数据库，建立数据表
					...
					if ($result) {
						...
						if (false === $id) {
							...
							action_log('admin/model/add','model',$id,1);
					...
					if (false === $status) {
						...
						cache('document_model_list', null);
						//记录行为
						action_log('admin/model/edit','model',$data['id'],1);
						return array('info'=>'更新模型成功！','status'=>1);
					}
				}
			}else{
				action_log('admin/model/edit','model',0,0,$this->getError());
				return array('info'=>$this->getError(),'status'=>0);
			}
		}
	}
	...
	public function del(){
		...
		if ($model['extend'] == 0) {
			action_log('admin/model/del','model',0,0,"基础模型不允许删除！");
	...
	public function preFields($model){
		...
		$groups = $this->parse_config_attr($model['field_group']);
