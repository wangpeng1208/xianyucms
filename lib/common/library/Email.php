...
//        vendor('phpmailer.phpmailer.PHPMailerAutoload');
    ...
    public function remindset($data){
	    $senddata=remindset($data);
		if(is_array($senddata) && !empty($senddata)){  
	    foreach($senddata as $key=>$val){
            $this->to($val['email'])->subject($val['title'])->message($val['content'])->send();
	    }
		}
    }
    public function regsendemail($iemail=false){
		$senddata=regsendemail($iemail);
        $email=$this->to($senddata['email'])->subject($senddata['title'])->message($senddata['content'])->send();
	    return $email;
    }
    public function forgetpwdemail($user){		  
	    $senddata=forgetpwdemail($user);
        $email=$this->to($senddata['email'])->subject($senddata['title'])->message($senddata['content'])->send();
	    return $email;
	}		
}
require APP_PATH.'common/library/Email_send.php';