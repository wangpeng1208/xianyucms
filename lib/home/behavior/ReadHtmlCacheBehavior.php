...
class ReadHtmlCacheBehavior{
    ...
    public function run(&$params){
        ...
        self::$controllerName = strtolower(request()->controller());
        self::$moduleName = strtolower(request()->module());
        // 开启静态缓存
        if('GET' === request()->method() && config('html_cache_on') && !(request()->instance()->isAjax())){
        ...
        // 分析当前的静态规则
         ...
         if(!empty($htmls)) {
            ...
            if(!empty($html)) {
				...
				//if(strrchr($rule,$html_file_suffix)!=$html_file_suffix){
				//$rule=$rule.'index'.$html_file_suffix;	
				//}
                ...
                define('HTML_FILE_NAME',RUNTIME_PATH.'/html/'.$theme_path.'/'.$rule.$html_file_suffix);
                return $cacheTime;
            }
        }
        // 无需缓存
    ...
    static public function checkHTMLCache($cacheFile='',$cacheTime='') {
        if(true == config('app_debug') || !is_file($cacheFile) || $cacheTime == 0){
            return false;
        }
		//永久缓存
		elseif($cacheTime<0){
        ...
        /* 
	    elseif(filemtime(think()->parseTemplateFile()) > $this->storage()->read($cacheFile,'mtime')) {
             // 模板文件如果更新静态文件需要更新
             return false;
        }  
		*/
