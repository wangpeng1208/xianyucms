...
'cms': {
	...
	'floatdiv': function() {
		$("<link>").attr({
			rel: "stylesheet",
			type: "text/css",
			href: cms.public + "cms/showfloatdiv/css/showfloatdiv.css"
		}).appendTo("head");
		$.getScript(cms.public + "cms/showfloatdiv/js/showfloatdiv.js", function() {});
	},
	//选项卡切换
	'tab': function() {
	...
	//公共
	'all': function(url){
		$('body').on("click", "#login,#user_login,#navbar_user_login,#vodRoomJoin", function(event){
			$('.xianyu-modal').modal('hide');
		...
		$('body').on("click", "#register", function() {
			xianyu.cms.modal(cms.root + 'index.php?s=user-reg-ajax');
			xianyu.user.reg();
		});
		//显示更多
...
'list': {
	...
	'url': function(url) {
		...
		$.get(url, function(data, status) {
			...
			if (xianyu.browser.language == 'zh-hk' || xianyu.browser.language == 'zh-tw') {
				$(document.body).s2t();
			}
		});
...
'detail': {
	...
	'download': function() {
		...
		if ($("#downlist").length) {
			$.getScript(cms.public + "cms/js/down.js");
		}
	},
},
'player': {
			...
			if($trysee > 0){
				window.setTimeout(function(){
					$.get(cms.root+'index.php?s=home-vod-vip-type-trysee-id-'+$vod_id+'-sid-'+$vod_sid+'-pid-'+$vod_pid, function(html){
						var index='<div class="embed-responsive embed-responsive-16by9"><div id="xianyucms-player-vip"><div class="xianyucms-player-box jumbotron">'+html+'</div></div></div>';
						$('#xianyucms_player').html(index);
						//$('.xianyucms-player-box').html(html).addClass("jumbotron");
						//xianyu.user.iframe();
						//$('#xianyucms-player-vip .xianyucms-player-iframe').hide();
					},'html');
				},1000*60*$trysee);
			}else{
				$('#xianyucms-player-vip .xianyucms-player-box').html($tips).addClass("jumbotron");
				$('#xianyucms-player-vip .xianyucms-player-iframe').hide();
			}
			//播放你密码
			...
			$('body').on("click","#user-price",function(){
				...
				$.get(cms.root+'index.php?s=home-vod-vip-type-ispay-id-'+$vod_id+'-sid-'+$vod_sid+'-pid-'+$vod_pid, function(json){
					if(json.status == 200){
						...
						$('#xianyucms-player-vip .xianyucms-player-box').html(json.info).addClass("jumbotron");
					}
				},'json');
			});				
		}else{
	...
	'get': function(t) {
		...
		$.getJSON(url, function(data) {
			...
			function barrager(){
				...
				$('#xianyucms_player').barrager(items[index]);
				if(t==0){
				index++;
				if (index == total) {
					clearInterval(looper);
...
'score': {
	'load': function() {
		 if (cms.id != undefined && cms.id != null && cms.id != '' && $('#xianyu-score').length > 0 && $('#xianyu-cm').length<=0) {
		   xianyu.score.ajax(cms.root + "index.php?s=home-mark-ajax-id-" + cms.id)
		 }
	},
	'loading': function() {
		  if (cms.id != undefined && cms.id != null && cms.id != '' && $('#xianyu-score').length > 0) {
		   xianyu.score.ajax(cms.root + "index.php?s=home-mark-ajax-id-" + cms.id)
		 }
	},	
     //加载评分与订阅收藏
...
'cm': {
	//按类型加载评论
	'load':function(){
		if($('#xianyu-cm[data-type=xianyu]').length){
			this.forum();
		}
		if($('#xianyu-cm[data-type=uyan]').length){
			this.uyan();
		}
		if($('#xianyu-cm[data-type=changyan]').length){
			this.changyan();
		}
	},	
	'forum': function(){
		    //如果同时需要评分并加载
		    if($('#xianyu-score').length > 0){
			xianyu.cm.ajax(cms.root + "index.php?s=home-cm-get-id-" + cms.id + "-sid-" + cms.sid);
			}else{
			xianyu.cm.ajax(cms.root + "index.php?s=home-cm-ajax-id-" + cms.id + "-sid-" + cms.sid);
			}
			xianyu.cm.emo();
	...
	'uyan': function(){
		$("#xianyu-cm").html('<div id="uyan_frame"></div>');
		$.getScript("http://v2.uyan.cc/code/uyan.js?uid="+$('#xianyu-cm[data-type=uyan]').attr('data-uyan-uid'));
	},
	'changyan': function(){
		$appid = $('#xianyu-cm[data-type=changyan]').attr('data-changyan-id');
		$conf = $('#xianyu-cm[data-type=changyan]').attr('data-changyan-conf');
		$sourceid = cms.sid+'-'+cms.id;
		var width = window.innerWidth || document.documentElement.clientWidth;
		if (width < 768) { 
			$("#xianyu-cm").html('<div id="SOHUCS" sid="'+$sourceid+'"></div><script charset="utf-8" id="changyan_mobile_js" src="https://changyan.sohu.com/upload/mobile/wap-js/changyan_mobile.js?client_id='+$appid+'&conf=prod_'+$conf+'"><\/script>');
		}else{
			$("#xianyu-cm").html('<div id="SOHUCS" sid="'+$sourceid+'"></div>');
					...
					$(".reply").click(function(e) {
						...
						if (!xianyu.user.islogin()) {
							...
							if ($("#rep" + curid).html() != '') {
								...
								if(!xianyu.user.islogin()){xianyu.user.loginform();return false;}
								$("#rep" + curid + " #comm-sub-form").xianyusub({
...
'user': {
	...
	'index': function() {
		...
		$('body').on("click", "#user-vip", function() {
			...
			xianyu.cms.modal(cms.root + 'index.php?s=user-buy-index');
		});
		//点击冲值影币
		...
		$('body').on("click", "#user-pay", function(e) {
			...
			if(type=='weixinpay'){
              e.preventDefault();
			  xianyu.cms.modal(cms.root + 'index.php?s=user-payment-index-pay_type-'+type+'-score_ext-'+score);
	...
	'payment': function(){
		...
		xianyu.cms.modal(cms.root + 'index.php?s=user-payment-index');
	},
	//检查VIP播放页面并刷新页面
	'iframe': function() {
		if ($("#xianyucms-player-vip").length > 0) {
			if ($(".xianyucms-player-iframe").length > 0 && $('.xianyucms-player-iframe').attr('src').indexOf("home-vod-vip-type-play") >= 0) {
				$('.xianyucms-player-iframe').attr('src', $('.xianyucms-player-iframe').attr('src')).show();
			} else {
	...
	'islogin': function() {
		var islogin = 0;
		if (document.cookie.indexOf('i7_token') >= 0) {
			islogin = 1;
			return true;
		}
		return false;
	...
	'loginform': function() {
		if (!xianyu.user.islogin()) {
			// xianyu.cms.modal(cms.root + 'index.php?s=user-qrcodelogin-qq');
			xianyu.cms.modal(cms.root + 'index.php?s=user-login-ajax');
			xianyu.user.login();
		} else {
			return false;
		}
	...
	'login': function() {
		$('body').on("click", "#login-submit", function() {
			if ("" == $("#username").val()){$.showfloatdiv({
				...
				if ("" != $("#password").val()) return $("#login-form").xianyusub({
					curobj: $("#login-submit"),
					txt: "数据提交中,请稍后...",
					isajax: 1,
					onsucc: function(a) {
	...
	'reg': function() {
		$('body').on("click", "#reg-submit", function() {
			...
			if ("" != $("#reg-form #password").val()) return $("#reg-form").xianyusub({
				curobj: $("#reg-submit"),
				txt: "数据提交中,请稍后...",
				onsucc: function(a) {
					if ($.hidediv(a), parseInt(a["rcode"]) > 0) {
	...
	'center': function() {
		...
		$.cxSelect.defaults.url = cms.public + "admin/xianyucms/libs/jquery-cxselect/js/cityData.json";
		...
		$('body').on("click", ".clearall", function() {
			...
			$.showfloatdiv({
				...
				succdo: function(r) {
					$("#pids").html(r.pids);
					xianyu.user.get(geturl);
				},
				txt: '确认删除?'
			});
		});
		xianyu.user.setuplabel();
		...
		$('body').on("click", "#add-feedback", function() {
			...
			xianyu.cms.modal(cms.root + 'index.php?s=user-center-addgb');
		});
		//修改密码
		$('body').on("click", "#user-email", function() {
			...
			xianyu.cms.modal(cms.root + 'index.php?s=user-center-iemail');
		});	
		//发送私信
		$('body').on("click", "#send-msg", function(){
			...
			xianyu.cms.modal(cms.root + 'index.php?s=user-home-msg-uid-'+uid);
		});			
		$('body').on("click", "#add-modal-submit", function(){
			var url = $('#modal-form').attr('action');
			$("#modal-form").xianyusub({
...
'gbook': { 
	...
	'load': function() {
		...
		$('body').on("click", "#gb-submit", function(){
			...
			if ("" != $("#gb_content").val()) return $("#gbook-form").xianyusub({
				curobj: $("#gb-submit"),
				txt: "数据提交中,请稍后...",
				onsucc: function(a) {
					if ($.hidediv(a), parseInt(a["rcode"]) > 0) {
					  xianyu.list.url(cms.root + "index.php?s=gb-show");
					} else - 3 == parseInt(a["rcode"])
				}
			}).post({
				url: cms.root + "index.php?s=/home/gb/adds"
			}), !1;
			$.showfloatdiv({
				txt: "输入留言内容"
...
'search': { //搜索
	'autocomplete': function(){
		var $limit = $('.xianyu_search').eq(0).attr('data-limit');
		if( $limit > 0){
			$.ajaxSetup({
				cache: true
			});
...
'image': {
	...
	'swiper': function(){	
		...
		$.getScript(cms.public+"cms/js/swiper.min.js", function(){	
var swiper=new Swiper('.box-slide',{pagination:'.swiper-pagination',lazyLoading:true,preventClicks:true,paginationClickable:true,autoplayDisableOnInteraction:false,autoplay:3000,loop:true,nextButton:'.swiper-button-next',prevButton:'.swiper-button-prev',});var swiper=new Swiper('.details-slide',{pagination:'.swiper-pagination',autoHeight:true,loop:true,nextButton:'.details-slide-next',prevButton:'.details-slide-pre',paginationType:'fraction',keyboardControl:true,lazyLoading:true,lazyLoadingInPrevNext:true,lazyLoadingInPrevNextAmount:1,lazyLoadingOnTransitionStart:true,});var swiper=new Swiper('.news-switch-3',{lazyLoading:true,slidesPerView:3,spaceBetween:0,nextButton:'.swiper-button-next',prevButton:'.swiper-button-prev',breakpoints:{1200:{slidesPerView:3,spaceBetween:0},992:{slidesPerView:2,spaceBetween:0},767:{slidesPerView:1,spaceBetween:0}}});var swiper=new Swiper('.news-switch-4',{lazyLoading:true,slidesPerView:4,spaceBetween:0,nextButton:'.swiper-button-next',prevButton:'.swiper-button-prev',breakpoints:{1200:{slidesPerView:4,spaceBetween:0},992:{slidesPerView:3,spaceBetween:0},767:{slidesPerView:2,spaceBetween:0}}});var swiper=new Swiper('.news-switch-5',{lazyLoading:true,slidesPerView:5,spaceBetween:0,nextButton:'.swiper-button-next',prevButton:'.swiper-button-prev',breakpoints:{1200:{slidesPerView:4,spaceBetween:0},992:{slidesPerView:3,spaceBetween:0},767:{slidesPerView:2,spaceBetween:0}}});var swiper=new Swiper('.vod-swiper-4',{lazyLoading:true,slidesPerView:4,spaceBetween:0,nextButton:'.swiper-button-next',prevButton:'.swiper-button-prev',breakpoints:{1200:{slidesPerView:4,spaceBetween:0},767:{slidesPerView:3,spaceBetween:0}}});var swiper=new Swiper('.vod-swiper-5',{lazyLoading:true,slidesPerView:5,spaceBetween:0,nextButton:'.swiper-button-next',prevButton:'.swiper-button-prev',breakpoints:{1200:{slidesPerView:4,spaceBetween:0},767:{slidesPerView:3,spaceBetween:0}}});var swiper=new Swiper('.vod-swiper-6',{lazyLoading:true,slidesPerView:6,spaceBetween:0,nextButton:'.swiper-button-next',prevButton:'.swiper-button-prev',breakpoints:{1200:{slidesPerView:5,spaceBetween:0},992:{slidesPerView:4,spaceBetween:0},768:{slidesPerView:3,spaceBetween:0}}});		
	...
	'lazyload': function(){
		...
		$.getScript(cms.public+"cms/js/jquery.lazyload.min.js", function(response,status){
			$(".loading").lazyload({
				effect : "fadeIn",
				failurelimit: 15
			}); 
	...
	'qrcode': function(){
	    if($(".qrcode")){
		$(".qrcode").append('<img class="qrcode" src="/?s=home-tools-qrcode&url='+encodeURIComponent(cms.murl)+'"/>');
		}
		$("#qrcode").popover({
				html: true
		});
		$("#qrcode").on('show.bs.popover', function () {
			$("#qrcode").attr('data-content','<img class="qrcode" src="/?s=home-tools-qrcode&url='+encodeURIComponent(cms.murl)+'"/>');
		})
	},
	//生成验证码图片
	'validate':function(){
...
'validate': {
	'load': function(){
		xianyu.validate.focus();
		xianyu.validate.click();
	},
	'focus': function(){//验证码框焦点
		$('body').on("focus", "#xianyu-validate", function(){
			$('#validate-xianyu,#validate-xianyu-reg').html(xianyu.image.validate());
			$(this).unbind();
		});
	},
	'click': function(){//点击刷新
...
'language':{//简繁转换
	's2t':function(){
		if(xianyu.browser.language=='zh-hk' || xianyu.browser.language=='zh-tw'){
			$.getScript(cms.public + "cms/js/s2t.min.js", function(data, status, jqxhr) {
				$(document.body).s2t();//$.s2t(data);
			});
		}
	},
	't2s':function(){
		if(xianyu.browser.language=='zh-cn'){
			$.getScript(cms.public + "cms/js/s2t.min.js", function(data, status, jqxhr) {
				$(document.body).t2s();//$.s2t(data);
			});
		}
...
xianyu.barrage.index();
xianyu.image.swiper();//幻灯片
xianyu.cms.floatdiv();//窗口提示信息
xianyu.cms.all();//主要加载
xianyu.cms.tab();//切换
xianyu.cms.collapse();
xianyu.cms.scrolltop();
xianyu.image.lazyload();//图片延迟加载
xianyu.search.autocomplete();//联系搜索
xianyu.image.qrcode();//二维码
xianyu.list.ajax();//列表AJAX
xianyu.detail.collapse();
xianyu.detail.playlist();//更多剧集
xianyu.detail.download();
xianyu.user.index();
xianyu.gbook.load();
xianyu.hits.load();
xianyu.validate.load();
xianyu.updown.load();
xianyu.score.load();//加载评分
xianyu.cm.load();//加载评论
xianyu.love.load();//订阅收藏
xianyu.playlog.load();//加载播放记录
//xianyu.language.s2t();//简体转繁体默认关闭需要开启去掉前面的//
