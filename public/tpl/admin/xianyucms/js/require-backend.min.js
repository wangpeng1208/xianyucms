...
//        'selectpage': ['css!../libs/selectpage/selectpage.css'],
    ...
    baseUrl: requirejs.s.contexts._.config.config.site.cdnurl + 'admin/xianyucms/js/', //资源基础路径
    map: {
        '*': {
...
define('fast',['jquery', 'bootstrap', 'toastr', 'layer', 'lang'], function ($, undefined, Toastr, Layer, Lang) {
    var Fast = {
        ...
        api: {
                ...
                options = $.extend({
                    ...
                    success: function (ret) {
                        ...
                        if (ret.code === 1) {
							...
							if(ret.data.indexOf("admin-html") > 0 ){
							parent.$('#html_create').attr('src',ret.data); 
							}
                        } else {
                    ...
                    error: function (XMLHttpRequest, textStatus, errorThrown){
                        Layer.close(index);
                        var ret = {code: XMLHttpRequest.status,msg:'请求返回'+XMLHttpRequest.status+'错误请使用浏览器调试查看具体错误', data: null};
                        Fast.events.onAjaxError(ret, error);
                    }
                }, options);
                $.ajax(options);
            ...
            //打开一个弹出窗口
            open: function (url, title,options,width,height,offset) {
                title = title ? title : "";
		...
		api: {
			...
			bindevent: function(table) {
				$(document).on("click", "[data-url].btn-detail,[data-url].btn-editone", function(e) {
					...
					Backend.api.open(url, name,'',width,height,offset);
				});
			...
			$(document).on('click', ".setstars", function() {
				...
				$.ajax({
					...
					success: function(data) {
						if (data.code === 1) {
							for (i = 1; i <= 5; i++) {
								$('#' + index[0] + '_' + index[1] + '_' + i).attr("src", requirejs.s.contexts._.config.config.site.cdnurl+"admin/xianyucms/img/star0.gif");
							}
							for (i = 1; i <= index[2]; i++) {
								$('#' + index[0] + '_' + index[1] + '_' + i).attr("src",requirejs.s.contexts._.config.config.site.cdnurl+"admin/xianyucms/img/star1.gif");
							}
							Toastr.success(data.msg ? data.msg : '操作成功');
						} else {
                            Toastr.error(data.msg ? data.msg : '操作失败');
            ...
            plupload: function (element, onUploadSuccess, onUploadError) {
                ...
                $(element, Upload.config.container).each(function () {
                    ...
                    Upload.list[id] = new Plupload.Uploader({
                        ...
                        flash_swf_url: '/tpl/admin/xianyucms/libs/plupload/js/Moxie.swf',
                        silverlight_xap_url: '/tpl/admin/xianyucms/libs/plupload/js/Moxie.xap',
                        filters: {
        ...
        events: {
            ...
            bindevent: function (form) {
			    $(document).on('click', ".addstars", function() {
				      ...
				      var index = arr.split("_");
				      for (i = 1; i <= 5; i++) {
					    $('#' + index[0] + '_' + i).attr("src", requirejs.s.contexts._.config.config.site.cdnurl+"admin/xianyucms/img/star0.gif");
				      }
				     for (i = 1; i <= index[1]; i++) {
					    $('#' + index[0] + '_' + i).attr("src",requirejs.s.contexts._.config.config.site.cdnurl+"admin/xianyucms/img/star1.gif");
			          }
